...
export function FeedbackDialog({ open, onOpenChange, messages, language }: FeedbackDialogProps) {
  ...
  const generateFeedback = async () => {
    ...
    try {
      ...
      if (!resp.ok) throw new Error("Failed to generate feedback");
      ...
      <DialogContent className="max-w-lg max-h-[85vh] w-[95vw] sm:w-full">
        <DialogHeader>
          <div className="flex items-center justify-between">
            <DialogTitle className="text-foreground">Performance Feedback</DialogTitle>
            {feedback && (
              <Button
                variant="outline"
                size="sm"
                className="gap-1.5 text-xs"
                onClick={() => downloadFeedbackPdf(feedback, messages, language)}
              >
